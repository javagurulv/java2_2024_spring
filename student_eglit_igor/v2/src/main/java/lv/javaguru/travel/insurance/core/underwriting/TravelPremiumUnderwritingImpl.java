...
public class TravelPremiumUnderwritingImpl implements TravelPremiumUnderwriting {
    ...
    private List<RiskDTO> calculateSelectedRisksPremium(AgreementDTO agreementDTO, PersonDTO person) {
        return selectedRisksPremiumCalculator.calculatePremiumForAllRisks(agreementDTO, person);
    }

    private BigDecimal calculateTotalPremium(List<RiskDTO> risks) {
        return risks.stream()
